...
| `_MAX_TPM_RETRIES` | `dana/core/openai_tool_bridge.py:47` | `2` | Groq 429 retry budget before degrading | — |
| `_MAX_REASONABLE_RETRY_AFTER_SEC` | `dana/core/openai_tool_bridge.py:52` | `45.0` | Ceiling on trusting Groq's own retry-after hint | `_LOCAL_TOOL_CALL_TIMEOUT_SEC` is derived from this — must stay coupled (see §2.2) |
| Groq default model | `dana/core/model_provider.py:337` | `"llama-3.3-70b-versatile"` | Fallback if `DANA_GROQ_MODEL` unset | Live deployments observed using `openai/gpt-oss-120b` via env override — the code default and the real default have drifted |
...
| Constant | File:Line | Value | Purpose | Fragility |
|---|---|---|---|---|
| `_LOCAL_TOOL_CALL_TIMEOUT_SEC` | `dana/core/react_dispatch.py:2151` | `_MAX_REASONABLE_RETRY_AFTER_SEC + 15.0` = **60.0** | Hard ceiling on one primary LLM turn | **Fixed this session** — used to be an independent literal `22.0`, which aborted mid-sleep during a legitimate Groq TPM throttle; now derived so the two can't drift apart again |
