...
def resolve_ticket(ticket, decision):
    ...
    ticket["status"] = (
        "APPROVED (Executed)" if decision == "Approve" else "DENIED (Failed Closed)"
    )
    result_log = f"Ticket {ticket['ticket_id']} has been {ticket['status']}."
    return result_log, ticket


# --- Tab 2: Vision & Screen Grounding Logic (ZeroGPU entrypoint) ---
@spaces.GPU
...
with gr.Blocks(title="Dānā · Agent Simulator") as demo:
    gr.Markdown(
        ...
        > **Local by design.** This Hugging Face Space runs a lightweight simulation of Dānā's **LangGraph routing corridor** and **Florence-2 UI grounding logic** (ZeroGPU-backed).
        > 
    ...
    gr.Button(
        "⬇️ Download Dānā for Windows (GitHub)",
            ...
            with gr.Row():
                ...
                with gr.Column():
                    annotated_out = gr.AnnotatedImage(
                        label="Florence-2 Bounding Box Annotation"
                    )
                    bbox_json = gr.Code(
                        label="Bounding Box JSON Output", language="json"
                    )
