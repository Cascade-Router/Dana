...
handoff are injected via callbacks (see ``dana.ui.notifications``).
...
def _pref_base_dirs() -> tuple[Path, Path | None]:
...
def _pref_path() -> Path:
    """User-local preference file (does not touch ``dana.paths``)."""
    primary, _legacy = _pref_base_dirs()
    return primary / _PREF_FILENAME


def _legacy_pref_path() -> Path | None:
    _primary, legacy = _pref_base_dirs()
    return (legacy / _PREF_FILENAME) if legacy is not None else None


def is_shell_watchdog_enabled() -> bool:
    """Return persisted enable flag; default off when unset/unreadable."""
    for path in (_pref_path(), _legacy_pref_path()):
        if path is None:
            continue
        try:
            if not path.is_file():
...
def get_shared_watchdog() -> ShellWatchdog:
    ...
    with _shared_lock:
        if _shared is None:
            ...
            try:
                from dana.ui.notifications import make_watchdog_error_handler
