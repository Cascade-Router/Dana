...
Run:
    python -m dana.middleware.actuator_executor
"""
...
from dana.middleware.resource_cap import apply_cpu_half_affinity
...
from dana.telemetry import log_actuator_done, log_actuator_start
...
def execute_tool_payload(
    ...
    *,
    ...
    if name == "type_stealth_text":
        from dana.operators.ghost_typist import type_stealth_text as _type_stealth
    ...
    if name == "navigate_and_click":
        from dana.operators.nav_and_click import navigate_and_click as _nav_click
    ...
    if name == "press_key":
        from dana.operators.keystroke import press_key as _press_key
    ...
    if name == "wake_cto":
        from dana.management.jason_supervisor import handle_wake_cto
...
def _maybe_pull_andon(
    *,
    ...
    try:
        from dana.management.jason_supervisor import trigger_andon_cord
...
def process_action(action: dict[str, Any], *, db_path: Any = None) -> dict[str, Any]:
    ...
    try:
        from dana.middleware.human_yield import start_human_yield_listener
    ...
    result = ""
    try:
        from dana.middleware.kill_switch import is_halted
    ...
    try:
        from dana.middleware.toast_notify import (
            format_actuator_toast,
            show_silent_toast_async,
        )
        from dana.telemetry import log_notification_toast
