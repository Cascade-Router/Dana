...
``proxy_server.cpp`` hardcodes its listen port at compile time
(``kListenPort``) — there is no CLI flag or env var to change it, and
``argv[1]`` is already reserved for the ONNX model path. This launcher's
default ``port`` must always match whatever that constant is currently
built as (8080 as of the sibling repo's latest local build) rather than
being independently configurable from this side.
"""
...
from dana.core.model_provider import ensure_dotenv_loaded
...
def start_cascade_proxy(port: int = 8080) -> subprocess.Popen | None:
    ...
    itself always binds whatever ``kListenPort`` was compiled with (see
    module docstring), so passing anything else here just makes the health
    check look in the wrong place.
    ...
    """
    # .env may not have been loaded into os.environ yet by whichever entry
    # point called us — the subprocess below only inherits real env vars
    # (GROQ_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, CASCADE_PROVIDERS, ...),
    # never .env's contents directly.
