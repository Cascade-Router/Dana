"""Jason supervisor → Dana coder LangGraph (conversational multi-agent handoff).

Topology (single pass — no agent ping-pong loops):
  START → jason_read → jason_evaluate → dana_code → END

Jason binds a read-only file tool on ``notes.txt``, evaluates the summary,
then hands a brief + file context to Dana. Dana binds the python_repl suite
...
from dana.tools.system_repl import file_editor, python_repl
...
def _evaluate_points(points: list[str]) -> str:
    ...
    if not points:
        return (
            ...
            "Dana should still emit a minimal mock metrics script."
        )
    checks = []
...
Generated by Dana under Jason supervisor delegation.
...
def jason_evaluate(state: JasonSupervisorState) -> dict[str, Any]:
    """Jason evaluates notes and builds the Dana handoff brief (no loop)."""
    ...
    brief = (
        "DANA HANDOFF (from Jason supervisor)\n"
    ...
    _log("agent=Jason handoff → Dana (brief ready)")
    return {
        "active_agent": "Jason",
...
def dana_code(state: JasonSupervisorState) -> dict[str, Any]:
    """Dana receives handoff; binds file_editor + python_repl; writes script."""
    ...
    _log(
        f"agent=Dana tools=['file_editor','python_repl'] "
    ...
    status = "DONE" if written else f"DANA_WRITE_FAIL: {write_obs[:200]}"
    _log(f"agent=Dana status={status}")
    return {
        "active_agent": "Dana",
        "script_path": script_path,
...
def dispatch_jason_supervisor_impl(query: str) -> str:
    ...
    if not final.get("script_written"):
        return (
            f"ERROR: Jason→Dana handoff incomplete status={final.get('status')} "
    ...
    return (
        ...
        f"{eval_line}; handed off to Dana; Dana wrote {script}."
    )
