...
local LLM, and overwrites ``donna/knowledge/feather_project_rules.md``.

Usage (vision poller running, Feather instructions visible on screen)::

    python -m donna.management.ingest_rules
"""
...
from donna.management.jason_supervisor import feather_project_rules_path
...
def acknowledge_ingestion(*, speak: bool = True) -> None:
    ...
    if not speak:
        return
    try:
        from donna.audio.multi_voice_tts import synthesize_speech
        from donna.ui.audio_mixer import play_donna

        wav = synthesize_speech(ACK_LINE, voice_id="donna")
