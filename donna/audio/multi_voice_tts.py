...
``donna`` / broker / llama prefer Piper (receptionist).
...
VoiceId = Literal["donna", "jason", "moa", "vision", "typist"]
...
try:
    from donna.core_agent import DEFAULT_PIPER_ONNX as _DEFAULT_PIPER_ONNX
except ImportError:
...
def uses_receptionist_piper(agent_id: str | None) -> bool:
    ...
    return resolve_voice_id(agent_id) == "donna"
    ...
    *,
    ...
    if vid == "donna":
        ...
        if _synthesize_piper(body, dest):
            return dest
        if _synthesize_pyttsx3(body, dest, prefer_male=False, rate=VOICE_RATES["donna"]):
            return dest
...
def _synthesize_piper(text: str, dest: Path) -> bool:
    ...
    if not PIPER_AVAILABLE or _DEFAULT_PIPER_ONNX is None:
        return False
    try:
        from donna.core_agent import get_piper_voice, synthesize_to_file
    except ImportError:
        return False
    try:
...
def svg_logo_loader_notes() -> dict[str, Any]:
    ...
    2. Pre-bake a circular logo PNG under ``donna/assets/`` and
       ``canvas.create_image(...)`` inside ``AssistiveTouchOrb._draw_orb``.
    3. Stage 8.9.9 default: LANCZOS PNG via ``donna.ui.logo`` + PhotoImage /
    ...
    """
    return {
        "preferred_asset": "donna/ui/assets/dana_logo_highres.png",
