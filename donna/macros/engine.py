...
Reuses ``donna.vision.florence_engine`` / Tracker capture helpers. All OS
...
_log = logging.getLogger("donna.macros.engine")
...
def default_macros_dir() -> Path:
    """Package-local macro JSON directory (``donna/macros/``)."""
...
def _default_click_fn(x: int, y: int) -> None:
    from donna.tools.os_control import click_left_sendinput, move_cursor_absolute
...
class MacroEngine:
    ...
    def save_macro(self, macro_sequence: MacroSequence) -> Path:
        """Persist sequence to ``donna/macros/<macro_id>.json`` (sanitized id)."""
        seq = (
            macro_sequence
